...
async def get_patient_appointments(
    ...
    try:
        ...
        logger.error(
            f"Error fetching appointments: {e}"
        )
            ...
            "data": [
                {
                    ...
                    "working_hours": (
                        f"{doctor.working_hours_start}"
                        f"-{doctor.working_hours_end}"
                    ),
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "last_interaction": (
                    patient.last_interaction.isoformat()
        ...
        logger.error(
            f"Error fetching patient info: {e}"
        )
...
async def get_session_info(
    ...
    try:
        session = await redis_memory.get_session(
            session_id
        )
    ...
    try:
        ...
        result = await db.execute(
            ...
            .join(
                ...
                == LatencyMetric.session_id,
            )
            .where(
                ConversationLog.patient_id
                == patient_id
            )
            .order_by(
                LatencyMetric.timestamp.desc()
            )
            .limit(limit)
        )
        ...
        logger.error(
            f"Error fetching latency stats: {e}"
        )
